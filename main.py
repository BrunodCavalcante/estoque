...
class Database:
    ...
    def log(self, acao, detalhes):
        self.execute(
            "INSERT INTO logs_sistema (acao, detalhes, data, data_iso) VALUES (?, ?, ?, ?)",
            (str(acao or ""), str(detalhes or ""), agora_ptbr(), data_iso_hoje()),
        )
...
class MainWindow(QMainWindow):
    ...
    def page_produtos(self):
        ...
        self.prod_qtd = QSpinBox(); self.prod_qtd.setRange(0, 10_000_000); self.prod_qtd.setPrefix(""); self.prod_qtd.setSpecialValueText("Quantidade atual")
        self.prod_min = QSpinBox(); self.prod_min.setRange(0, 10_000_000); self.prod_min.setSpecialValueText("Quantidade mínima / média mensal")
        ...
        self.entrada_qtd = QSpinBox(); self.entrada_qtd.setRange(1, 10_000_000); self.entrada_qtd.setSpecialValueText("Quantidade que chegou")
    ...
    def page_retirada(self):
        ...
        self.ret_qtd = QSpinBox(); self.ret_qtd.setRange(1, 10_000_000); self.ret_qtd.setSpecialValueText("Quantidade")
    ...
    def carregar_historico(self):
        hist = self.db.rows("SELECT * FROM movimentacoes ORDER BY id DESC")
        self.tabela_historico.set_data([[h['tipo'], h['produto'], h['pessoa'], h['quantidade'], h['data']] for h in hist])

    def salvar_produto(self):
        nome, cat = self.prod_nome.text().strip(), self.prod_categoria.text().strip()
        qtd, qtd_min = self.prod_qtd.value(), self.prod_min.value()
    ...
    def editar_produto(self, row):
        ...
        self.prod_nome.setText(p['nome']); self.prod_categoria.setText(p['categoria']); self.prod_qtd.setValue(int(p['quantidade'] or 0)); self.prod_min.setValue(int(p.get('quantidade_minima') or 0)); self.btn_prod_salvar.setText("Atualizar")

    def editar_produto_selecionado(self): self.editar_produto(self.selected_row(self.tabela_produtos))

    def cancelar_produto(self):
        self.produto_edit_id = None; self.prod_nome.clear(); self.prod_categoria.clear(); self.prod_qtd.setValue(0); self.prod_min.setValue(0); self.btn_prod_salvar.setText("Salvar")
    ...
    def entrada_estoque(self):
        produto, qtd = self.entrada_prod.currentData(), self.entrada_qtd.value()
        ...
        self.carregar_produtos(); self.carregar_historico(); self.info("Estoque atualizado")
    ...
    def retirada(self):
        produto, pessoa, qtd = self.ret_prod.currentData(), self.ret_pessoa.currentData(), self.ret_qtd.value()
        ...
        cur = self.db.conn.cursor(); cur.execute("UPDATE produtos SET quantidade=quantidade-? WHERE nome=?", (qtd, produto)); cur.execute("INSERT INTO movimentacoes (tipo, produto, pessoa, quantidade, data) VALUES (?, ?, ?, ?, ?)", ("RETIRADA", produto, pessoa, qtd, agora_ptbr())); self.db.conn.commit()
        self.db.log("Retirada de produto", f"Produto: {produto} | Pessoa: {pessoa} | Quantidade: {qtd}")
        self.ret_prod.setCurrentIndex(0); self.ret_pessoa.setCurrentIndex(0); self.ret_qtd.setValue(1); self.carregar_produtos(); self.carregar_historico(); self.info("Retirada realizada")
    ...
    def gerar_relatorio_logs(self):
        ...
        texto = f"Log do sistema - {d}/{m}/{a}\nGerado em: {agora_ptbr()}\nTotal de alterações no dia: {len(logs)}\n\nData e horário | Alteração | Detalhes\n" + "-"*100 + "\n"
        texto += "\n".join(f"{l['data']} | {l['acao']} | {l['detalhes']}" for l in logs) or "Nenhum registro encontrado para esta data."
        self.relatorio_texto.setPlainText(texto)

    def imprimir_relatorio(self):
        if not self.relatorio_texto.toPlainText().strip(): return self.erro("Gere o relatório antes de imprimir.")
        printer = QPrinter(QPrinter.PrinterMode.HighResolution)
        dialog = QPrintDialog(printer, self)
        if dialog.exec(): self.relatorio_texto.print(printer)
    ...
    def importar_backup(self):
        ...
        try:
            ...
            for l in backup.get('logs_sistema', []): cur.execute("INSERT INTO logs_sistema (id,acao,detalhes,data,data_iso) VALUES (?,?,?,?,?)", (int(l.get('id') or 0), str(l.get('acao','')), str(l.get('detalhes','')), str(l.get('data','')), str(l.get('data_iso') or parse_data_ptbr(l.get('data')) or data_iso_hoje())))
